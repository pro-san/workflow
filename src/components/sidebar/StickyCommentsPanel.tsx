...
import { exportCommentsToCSV, printCommentsPDFReport } from '../../utils/exportFeedback';
import {
  MessageSquare,
  Plus,
  CheckCircle2,
        ...
        <div className="flex items-center space-x-1.5">
          ...
          <div className="relative">
            <button
              ...
              className="bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-200 disabled:opacity-40 disabled:hover:bg-slate-800 disabled:cursor-not-allowed px-2 py-1 rounded-md text-[11px] font-medium flex items-center space-x-1 transition-colors shadow-sm"
              title={comments.length > 0 ? "Export comments as CSV or PDF report" : "No comments to export"}
            >
            ...
            {showExportMenu && comments.length > 0 && (
              <>
                <div
                  className="fixed inset-0 z-40"
                  onClick={() => setShowExportMenu(false)}
                />
                <div className="absolute right-0 top-full mt-1.5 w-48 bg-slate-950 border border-slate-800 rounded-lg shadow-2xl z-50 p-1 divide-y divide-slate-800/80 text-xs">
                  <div className="py-0.5">
                    <button
                      onClick={() => {
                        exportCommentsToCSV(project);
                        setShowExportMenu(false);
                      }}
                      className="w-full px-2.5 py-1.5 flex items-center space-x-2 text-slate-200 hover:bg-amber-500/20 hover:text-amber-200 rounded text-left transition-colors"
                    >
                      <FileText className="w-3.5 h-3.5 text-emerald-400 flex-shrink-0" />
                      <div>
                        <p className="font-medium leading-none">Export as CSV</p>
                        <p className="text-[10px] text-slate-400 mt-0.5">Spreadsheet tabular data</p>
                      </div>
                    </button>
                  </div>
                  <div className="py-0.5">
                    <button
                      onClick={() => {
                        printCommentsPDFReport(project);
                        setShowExportMenu(false);
                      }}
                      className="w-full px-2.5 py-1.5 flex items-center space-x-2 text-slate-200 hover:bg-amber-500/20 hover:text-amber-200 rounded text-left transition-colors"
                    >
                      ...
                      <div>
                        <p className="font-medium leading-none">Export PDF Report</p>
                        <p className="text-[10px] text-slate-400 mt-0.5">Formatted printable audit</p>
                      </div>
                    </button>
                  </div>
                </div>
